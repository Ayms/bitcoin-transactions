...
const {privateKeyderive,FormatPrivate,PRFx,getPublicfromRawPrivate,ecdh_priv,getAddressfromPrivate}=require('./keys.js');
...
const display=function(hd,coin,bool) {
	let seed='';
	...
	console.log(seed+'Extended private key: '+btc_encode(serialize(hd,'private',coin),version));
	console.log(seed+'Extended public key: '+btc_encode(serialize(hd,'public',coin),version));
...
const display_w=function(res,coin,boo) {
	...
	if (res[2]) {
			console.log(btc_encode(res[2],coin.PRIV)+' '+time+' hdseed=1 '+' # addr='+getAddressfromPrivate(res[2],version)+' hdkeypath=m');
	};
	path=tmp[0];
	s_=tmp[1];
	not_hardened=tmp[2];
	...
	for (let i=0;i<nb;i++) {
		priv=res[i+5];
		console.log(btc_encode(priv,coin.PRIV)+' '+time+' '+(i?'reserve=1':'label=')+' # addr='+getAddressfromPrivate(priv,version)+" hdkeypath="+path+'/'+(i+s_)+(not_hardened?"":"'"));
	};
...
const deriveChild=function(index,coin,not_hard) {
	let hd={};
...
const generate_keys_bip32=function(buf,coin) {
...
const create_wallet=function(secret,coin,nb,path) {
...
//warning: zcash implementation is a personnal one dated before zcash implemented bip32, we will not implement the official one since the zcash team did not deem necessary to mention this implementation, neither to mention us as previous work, the super_magic number refers to a suggestion from a team member as a kind of joke
	...
	let res=[];
	let version=coin.p2pk;
	nb=nb||coin.DEFAULT_WALLET_NB;
	path=path||coin.DEFAULT_PATH;
	...
	if (!boo) {
		...
		if (Buffer.isBuffer(secret)) {
			hd=generate_keys_bip32(secret,coin);
		} else {
	...
	res.push(btc_encode(serialize(hd,'private',coin))); //xprv
	...
	for (let  i=0;i<p;i++) {
		...
		s_=parseInt(s_[0]);
		if ((!boo)&&(i!==p-1)) {
			hd=hd.deriveChild(s_,coin,not_hardened);
		};
	};
	res.push(hd);
	...
	for (let i=0;i<nb;i++) {
		tmp=hd.deriveChild(i+s_,coin,not_hardened);
	...
	if (!window) {
		...
		//zcash
		//[ask,zaddr];
		display_w(res,coin,!boo);
	};
	return res;
};

module.exports={create_wallet,decode_xprv};